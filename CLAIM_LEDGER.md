...
| Production metal `op=recover` is an ESTOP-ack analog from untrusted autonomy IPC; it is not authenticated operator recovery and not integrity unlatch | Same-process | ACTIVE | `EstopLatch::is_integrity_abort` is independent of `engaged`; `clear_estop_requires_recovery_at` emits `recovery_refused` / `integrity_abort_requires_online_restart` before `Plant::clear_estop`; tests `recover_after_unknown_outcome_refuses_before_clear_estop`, `recover_after_replay_refuses_and_fresh_id_cannot_execute`, `recover_after_integrity_then_engage_estop_still_refused`, `recover_after_ordinary_estop_clears_and_allows_new_command`. Does **not** claim process-isolation or metal MEASURED. |
| Authenticated operator recovery channel separate from autonomy IPC | Process-isolation | **NAMED_HOLE** | metal `recover` calls `clear_estop_requires_recovery_now(true)` with no OS-group / Unix-socket / recovery-token check |
| PRE-FIX physical baseline SHA `2f68a5d82fec5e7e2c0b78ca88fe65d8a8a4acfc` is intentionally vulnerable to recover-integrity and is retained only as frozen pre-fix baseline | Physical safety | **NOT_EVIDENCE** | immutable freeze; do not rewrite; Run A runbook `docs/metal/RUN_A_PRE_FIX_BASELINE.md` |
| PATCHED recover/integrity latch `709a0793fcf5e1706b059c27f1ed5a8dba0ca1f1` | Same-process | ACTIVE | governor tests; LINUX PTY STATUS passed on Actions `35322120250` test-metal (not metal); REAL XL330 STATUS not measured; Run B `docs/metal/RUN_B_PATCHED_AUTHORITY.md` |
