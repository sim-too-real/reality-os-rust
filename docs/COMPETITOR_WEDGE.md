...
| **Autoware** | Apache-2.0, AS-IS, no fitness warranty. In-stack fail-safe AD API (normal → Minimal Risk Maneuver). 2026 Autoware Foundation blueprint: independent Safety Island on **Zephyr RTOS**, Autoware on Linux, authority transfer on fault. | Closest *placement* analog (separate island). Vehicle stack; not cobot identity/PFL; island not verified as a shipped certified last-gate. |
