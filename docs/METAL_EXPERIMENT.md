...
| Interface | Dynamixel Protocol 2.0, default **57 600** 8N1 on `/dev/ttyUSB*` (probe also tries 115 200, 1 Mbps, 2 Mbps, 3 Mbps, 4 Mbps, and Wizard **9 600**) |
...
* Linux asserts DTR on the first USB-serial open. Cheap FTDI/CP2102 boards wire that to servo RESET. The driver and campaign clear HUPCL before exclusive open so close/crash-replay does not reboot; real UART waits 500 ms after open. U2D2 has no DTR-RESET. Do not toggle DTR/RTS from userspace. A half-duplex adapter that needs a longer reboot than 500 ms is still a first-contact hole.
...
* `udevadm trigger` / ModemManager stop can re-enumerate FTDI as `ttyUSB1`. The campaign records the USB serial, installs an `ATTRS{serial}` udev rule (not `KERNEL==ttyUSB0` only), and continues on `/dev/serial/by-id` when that symlink exists.
