...
| Interface | Dynamixel Protocol 2.0, default **57 600** 8N1 on `/dev/ttyUSB*` (probe also tries 115 200, 1 Mbps, 2 Mbps, 3 Mbps, and 4 Mbps) |
...
* CI `os-users` runs `scripts/metal-pty-sequence.sh` so campaign-script first-contact bugs (empty `METAL_CMD_ID`, crash-replay hang, replay latch, identify-only probe) fail before the XL330 bench. That job is not physical evidence.
