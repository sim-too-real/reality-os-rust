...
* USB autosuspend (often 2 s on Ubuntu) can idle the adapter between campaign cases and miss the 40 ms live deadline. The campaign writes `power/control=on` on the tty's USB device. PTY has no sysfs node.
