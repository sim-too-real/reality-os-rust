...
* Observed motion is measured from `bus/present` / `bus/goal` (device registers), not inferred from IPC status. `bus/present` is the last acquired sample (pre-write during propose). After an authorized hold/nudge the campaign waits until XL330 **Moving** (addr 122, persisted as `bus/moving`) is 0, then re-acquires so `valid_nudge` records a present change; a goal write against a stale cache is not motion. A fixed 300 ms sleep can sample while the profile (velocity 20 + accel 10 ≈ 230 ms for 32 ticks) is still traveling. The certified nudge is `action=0.2` (the full 32-tick / ~2.8° cap) so plastic-gear backlash is less likely to hide the step.
...
* After an authorized write the campaign waits for Moving (addr 122) = 0 (1.5 s cap) instead of a fixed 300 ms sleep, then samples present. Sensor persists that register as `bus/moving`.
