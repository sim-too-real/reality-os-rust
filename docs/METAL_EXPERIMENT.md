...
`realityos-metal-smoke serve` calls `RuntimeSession::start_online` → `OsMonotonicClock`. It does not use the HIL `Authority` object or `FakeClock`. Idle serve pets the 50 ms software watchdog about every 40 ms and heartbeats about every 800 ms (each emit fsyncs journal+seal; a 10 ms pet of both was ~400 fsyncs/s and can miss the 100 ms watchdog). Propose pets again between sensor acquire and the certified write. After identify, live I/O is one attempt at 40 ms, a live timeout does not retry 16 times, and sensor sampling is a single 26-byte RAM block. Goal writes reuse the last sensor present and skip redundant profile/torque pokes. `scripts/metal-kill-serve.sh` kills leftover `serve` processes for a metal root so a zombie cannot rewrite journal+seal after `rm -rf` and poison `--first-online`.
