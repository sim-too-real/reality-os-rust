...
`realityos-metal-smoke serve` calls `RuntimeSession::start_online` → `OsMonotonicClock`. It does not use the HIL `Authority` object or `FakeClock`. `new_online` stamps the start watchdog after the heartbeat journal persist so persist latency is not counted as a 100 ms miss (driver/deployment cannot catch that up). Idle serve pets the 50 ms software watchdog when the authority-clock gap since the last tick stamp reaches about 40 ms, and heartbeats about every 800 ms (each emit fsyncs journal+seal; a 10 ms pet of both was ~400 fsyncs/s and can miss the 100 ms watchdog). The idle loop accepts a waiting IPC client *before* the idle heartbeat; a heartbeat persist in the same iteration as `handle` can consume the 100 ms window (refuse at the first handle pet, empty `serve.err`). After heartbeat persist, serve refreshes the watchdog so the next accept does not inherit that gap. Propose ticks the watchdog once on IPC accept; it does not heartbeat or re-pet between sensor acquire and the certified write (`dispatch_issued` ticks immediately before `write_online`). After the write, serve refreshes the watchdog (so the next propose does not inherit prepare/consume/emit fsync time) and then persists freshness. After identify, live I/O is one attempt with a **40 ms overall deadline** (dribbled 4×40 ms reads are 160 ms and miss the watchdog). Sensor sampling is a single 26-byte RAM block. Propose pets the watchdog again after acquire so dispatch does not inherit that read. Setup enables torque after the EEPROM settle so the first certified write is only `goal_position` (not torque-on + goal). Goal writes reuse the last sensor present and skip redundant profile/torque pokes. `scripts/metal-kill-serve.sh` kills leftover `serve` processes for a metal root so a zombie cannot rewrite journal+seal after `rm -rf` and poison `--first-online`.

Keep `REALITYOS_METAL_ROOT` on tmpfs (the campaign default is `/tmp/realityos-metal`). Each watchdog/heartbeat emit fsyncs journal+seal; if a single fsync takes more than 100 ms the software watchdog latches and cannot be caught up. That is a deployment constraint, not a certified hardware watchdog.
