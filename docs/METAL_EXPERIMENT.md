...
Model and firmware are latched at identify time. The motion-block sensor packet does not contain those EEPROM fields and does not invent them. On live I/O each sensor also re-reads model/firmware/id (addr 0, 8 bytes) so a physical servo swap on the same USB-UART updates `probe_identity` before write-time `verify_live_hardware`. Identical model+fw units remain indistinguishable (XL330 EEPROM has no factory serial). Campaign `hot_swap.json` still overlays identity without changing EEPROM.
...
* Live sensor re-reads model/firmware/id (addr 0, 8 bytes). A physical XL330 swap on the same USB-UART used to keep the latched firmware, so `verify_live_hardware` would still match. The motion block does not carry those fields. Identical model+fw units still cannot be distinguished.
