...
* Half-duplex turnaround: after `flush()`, a real UART sleeps 500 µs before the first read so a cheap TTL/RS485 DE/RE can release the line (broadcast sniff, SRL poke, and ID-register reads included). A PTY skips the sleep. U2D2 handles turnaround internally. Do not toggle RTS (DTR-RESET risk).
