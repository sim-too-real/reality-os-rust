...
* Crash-replay must not `wait $AUTH_PID` unless the smoke child actually exited. `after_write_before_ack` / `after_ack` fire only after `act()` returns Ok; a refused propose leaves serve up. Propose IPC has a 20 s I/O timeout so a wedged serve cannot hang the campaign.
