...
* Setup refuses torque-on when Present Input Voltage is outside Wizard min/max voltage EEPROM (`dxl_vin_outside_wizard_limits`). It does not widen those limits. A 0 or failed VIN read used to skip that check; setup now refuses (`dxl_vin_unreadable_before_torque`). Broadcast sniff does not take `TIOCEXCL` so probe cannot steal exclusive from the next serve open.
