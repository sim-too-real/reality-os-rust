...
pub fn default_unresolved() -> Vec<String> {
    vec![
        ...
        "Wizard Startup Configuration bit 0 torque-ons after DTR-RESET and tracks Goal (RAM initial 0). Open broadcasts torque-off during the 500 ms settle at the current baud and identify directed-torque-offs before model READs; serve writes factory 0 so the next crash_if open does not yank. Probe does not write that EEPROM. A Wizard non-57600 bus still tracks during the first factory-baud settle".into(),
