...
pub fn default_unresolved() -> Vec<String> {
    vec![
        ...
        "Wizard Startup Configuration bit 0 torque-ons after DTR-RESET and tracks Goal (RAM initial 0). Open broadcasts torque-off during the 500 ms settle, alternating the open baud with 115200 on the held fd so a Wizard-rate bus is not left tracking until discover retunes; restore to the open baud fails closed (an ignored leftover 115200 fd missed a factory 57600 servo and walked into the automatic 1 Mbps open). Identify directed-torque-offs before model READs; serve writes factory 0 so the next crash_if open does not yank. A failed setup read of Startup Configuration / Position I/D / feedforward / Bus Watchdog used to look like factory 0 and skip that write; setup now writes unless the read is already the safe value and fails closed unless readback matches. Probe does not write that EEPROM. 1 Mbps is not spoken during settle (CH340 wedge). A Wizard 2/3/4 Mbps bus still tracks during that window".into(),
        ...
        "REALITYOS_METAL_BAUD / SERVO_ID are probe scan extras; init/probe persist only the device path until discover writes the measured pair (a leftover 2/3/4 Mbps env used to land in metal.json before identify and wedge CH340 on the after-scan retry). serve keeps the pair probe wrote".into(),
