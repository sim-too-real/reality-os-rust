...
pub fn default_unresolved() -> Vec<String> {
    vec![
        ...
        "a half-duplex TTL/RS485 adapter that needs more than 1.5 ms after host TX, or more than 500 ms after DTR-RESET, is still a first-contact hole. INST_REBOOT identify now drains leftover Reboot status (same shape as PING; PTY ClearBuffer is a no-op) and polls a short ping up to 1.5 s (a single 400 ms wait missed a 500–800 ms XL330; setup recv_status is 16×150 ms, so one identify used to sit through the deadline and then accept a later reply). An adapter that stays silent longer than 1.5 s after Reboot still fail-closes".into(),
