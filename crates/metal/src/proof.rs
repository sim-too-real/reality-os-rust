...
pub fn default_unresolved() -> Vec<String> {
    vec![
        ...
        "a USB-UART with no adapter serial (typical CH340/CP2102) is rebound by vid:pid:devpath / by-path, not KERNEL==ttyUSB0 or a parent hub serial; two adapters on the same USB port path are indistinguishable".into(),
