...
pub fn default_unresolved() -> Vec<String> {
    vec![
        ...
        "HUPCL is cleared on the device node after a shared serialport open, then exclusive is taken; stty after TIOCEXCL is EBUSY on the node and on /proc/<pid>/fd/N, /proc/self/fd/N misses an O_CLOEXEC tty, and a pre-open stty is lost because a fresh USB-serial session restores kernel-default HUPCL".into(),
