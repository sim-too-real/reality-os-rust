...
pub fn default_unresolved() -> Vec<String> {
    vec![
        ...
        "HUPCL is cleared on the held fd via /proc/self/fd/N after exclusive serialport open; stty -F on the device node is EBUSY under TIOCEXCL, and a pre-open stty is lost because a fresh USB-serial session restores kernel-default HUPCL".into(),
