...
pub fn default_unresolved() -> Vec<String> {
    vec![
        ...
        "first USB prepare fails closed until the UART sysfs node has a non-empty USB serial or busnum:devpath:vid:pid, then waits briefly for iSerial and locks that identity; an empty CH340 serial file, a parent hub serial, inventing 0:nodevpath, waiting for idVendor alone, or promoting a late FTDI serial after a dest-only bind used to miss when serve measured a different serial".into(),
