...
pub fn default_unresolved() -> Vec<String> {
    vec![
        ...
        "every campaign restart (first serve, disconnect restart, crash-replay bind/restart/reset-hold, live USB replug) retries start_auth until a sensor JSON body is ok=true; propose/sensor exits 0 for ok=false so a DTR-RESET refuse used to look like a landed sample. A not-yet-live first bind already created driver.jsonl; retrying --first-online then died as first_online_but_journal_or_seal_exists. Retries now use --restart when the journal or seal exists (scripts/metal-online-journal.sh); they do not delete the journal to fake first boot. Authorized settle timeout (1.5s) fails the campaign instead of sampling a traveling horn".into(),
