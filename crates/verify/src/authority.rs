...
use realityos_plant::{ActionParams, HardwareBackedPlant, HardwareDriverPort, Plant};
...
pub struct SimAuthority<P: HardwareDriverPort> {
    ...
    seen_observation_ids: HashSet<String>,
...
impl<P: HardwareDriverPort> SimAuthority<P> {
    pub fn open(
        ...
        Ok(Self {
            ...
            seen_observation_ids: HashSet::new(),
            last_obs_ts: None,
            sequence: 0,
            freshness_s: 0.25,
    ...
    pub fn restart(
        ...
        let next = Self::open(plant, manifest, now_s)?;
        self.ros = next.ros;
    ...
    pub fn screen_proposal(
        ...
        if self.seen_command_ids.contains(&cid) {
    ...
    pub fn decide_and_maybe_write(
        ...
        if executed {
            self.seen_command_ids.insert(command_id.clone());
